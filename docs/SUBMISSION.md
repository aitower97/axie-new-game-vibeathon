# Tactic Dice — Submission copy (Axie Vibeathon 2026, Round 1)
...
**Progression.** In the Lab you spend Essence to evolve a part (irreversible) or lock parts for free to concentrate the die on the faces you want. Research and Resources screens sketch the meta loop.
...
Desktop, mouse. Enter the hub → PVE → **Roll dice** → click one of your Axies → click a tile to move → click an enemy to attack (Basic or Special) → **Pass turn**. Drag to pan the camera, wheel to zoom. Evolve parts in the Lab between matches. Full guide: `docs/HOW_TO_PLAY.md`.

## Axie Core Fit

The body parts are not skins or a stat sheet: they are the six faces of the die that runs the whole game. Axie's genome — six parts plus a class — becomes a probability distribution the player shapes by evolving and locking parts. Real part names, real 3D parts from the Axie Origins kit, the class triangle and a class-based crit profile all come from Axie. If you remove the Axie parts, there is no die and no game.

## Current Prototype

- 8×7 3D board (Three.js) with terrain: stone, water, slow zones, low obstacles.
- Three playable classes (Beast, Bird, Aqua) with 12 real Axie parts as die faces.
- Lord with its own die (attack, shield, mark, heal, buff, duplicate).
...
- Lab (evolve / lock), Research and Resources screens.
